...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Québecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Québecoin Signed Message:\n";
...
{
    // Québecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420480); // Québecoin: 420480 blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // Québecoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // Québecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Québecoin - DarkGravity v2, written by Evan Duffield - evan@darkcoin.io */
...
{
    // Québecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xd3, 0xed, 0xc9, 0xf1 }; // Québecoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QuébecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QuébecoinMiner : ProcessBlock, block not accepted");
    }
